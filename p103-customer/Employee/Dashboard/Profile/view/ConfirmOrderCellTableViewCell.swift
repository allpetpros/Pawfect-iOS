//
//  ConfirmOrderCellTableViewCell.swift
//  p103-customer
//
//  Created by Daria Pr on 08.06.2021.
//  Copyright © 2021 PULS Software. All rights reserved.
//

import UIKit

class ConfirmOrderCellTableViewCell: UITableViewCell {
...
private extension ConfirmOrderCellTableViewCell {
    func setupLayout() {
...
extension ConfirmOrderCellTableViewCell {
    ...
    func historyOrdersSetup(orders: EmployeeHistoryDetails) {
        ...
        timeLabel.text = "\(toDate(millis: Int64(orders.timeFrom))) : \(toDate(millis: Int64(orders.timeTo)))"
    }
    
    func confirmedOrdersSetup(orders: EmployeeConfirmedItems) {
        stackView.arrangedSubviews.forEach { stackView.removeArrangedSubview($0);$0.removeFromSuperview() }
        ...
        nameOwnerLabel.text = "\(orders.customer.name) \(orders.customer.surname)"
        if let avatar = orders.customer.imageUrl {
            petOwnerImageView.sd_setImage(with: URL(string: avatar))
        }
        timeLabel.text = "\(toDate(millis: Int64(orders.dateFrom))) : \(toDate(millis: Int64(orders.dateTo)))"
    }
}
}

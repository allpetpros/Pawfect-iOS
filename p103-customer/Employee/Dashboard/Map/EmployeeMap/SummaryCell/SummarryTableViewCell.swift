...
class SummarryTableViewCell: UITableViewCell {
    //MARK: - IBOutlet
    ...
    override func awakeFromNib() {
        super.awakeFromNib()
        summaryDataTableView.register(UINib(nibName: "SummaryDataCell", bundle: nil), forCellReuseIdentifier: "SummaryDataCell")
    ...
    func setupUI() {
        ...
        doneButtonTapped.setTitle("Send Summary", for: .normal)
...
extension SummarryTableViewCell: UITableViewDelegate,UITableViewDataSource {
    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        return summaryArray.count
    }
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell = summaryDataTableView.dequeueReusableCell(withIdentifier: "SummaryDataCell") as! SummaryDataCell
        cell.actionLabel.text = summaryArray[indexPath.row].name
        cell.durationLabel.text = CommonFunction.shared.toDate(millis: Int64(summaryArray[indexPath.row].starttime))
        return cell
    }
}
